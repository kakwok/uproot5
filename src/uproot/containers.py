...
def _read_nested(
    ...
    if isinstance(model, numpy.dtype):
        ...
        if isinstance(model, AsContainer):
            #number = context["forth"].get_current_counter()
            for i in range(length):
                # context["forth"].set_current_counter(number)
                values[i] = model.read(
    ...
    def read(self, chunk, cursor, context, file, selffile, parent, header=True):
        ...
        if is_memberwise:
            ...
            if helper_obj.is_forth():
                ...
                #print(temp_node, temp_node_top, temp_form, temp_form_top, 'PPOPOPOO')
            keys = _read_nested(
                self._keys,
                length,
                chunk,
            ...
            if helper_obj.is_forth():
                ...
                keys_node, keys_node_top, keys_form, keys_form_top = forth_obj.replace_form_and_model(None, {"name": "TOP", "content": {}})
                print(keys_form, keys_node_top, "OOKKOKOKOk")
